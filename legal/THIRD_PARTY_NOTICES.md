...
이 문서는 공개 배포·유료 판매 시 설치본·포터블·About에 함께 둘 **고지 초안**입니다.  
법률 자문이 아니며, 배포 전에 라이선스 원문을 한 번 더 확인하세요.

앱 자체 라이선스(저장소 `package.json`): **MIT**  
...
유료 앱에 Electron을 포함하는 것은 일반적이며, **고지 파일을 배포물에서 제거하지 말 것**.
...
MIT·ISC는 **상업적 이용·판매 가능**. 조건은 주로 **저작권·라이선스 고지 유지**.
...
- [ ] 설치 UI `License.rtf`에 앱 MIT + 7-Zip LGPL + Electron + 공공데이터 출처 요약 포함  
- [ ] 설치/포터블에 `THIRD_PARTY_NOTICES.md`(본 문서) 또는 동등 고지 포함  
- [ ] `resources/7zip/LICENSE.txt` 유지, `7za.exe` 교체 가능 구조 유지  
- [ ] Electron/Chromium LICENSE 파일이 배포물에 존재하는지 확인  
- [ ] About(트레이 「정보」)에 출처·고지 안내  
- [ ] 공식 기관 사칭·오인 소지 문구 제거  
- [ ] (선택) `license-checker`로 의존성 라이선스 전수 확인  

**참고:** LGPL `7za`를 별도 프로세스로 쓰는 현재 구조에서는, 앱 전체를 GPL/LGPL로 공개해야 하는 의무가 일반적으로 발생하지 않습니다. (개조·정적 링크 시 달라질 수 있음)
...
Commercial sale is generally permitted under these licenses if attribution and LGPL obligations for 7-Zip are met.

---

*초안 작성: Neo Desktop Calendar 배포 컴플라이언스용. 필요 시 변호사 검토를 권장합니다.*
