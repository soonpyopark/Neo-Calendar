...
/** Colored tag glyph(s) for event bars / list rows / detail (MDC). */
export function EventTagIcons({
  event,
  tags = [],
  className,
  max = 3
}: EventTagIconsProps): ReactElement | null {
  ...
  const names = resolved.map((tag: { name?: string }) => tag.name).join(', ')

  return (
    <span className={cn('event-tag-icons', className)} title={names} aria-label={`태그: ${names}`}>
      {visible.map((tag: { id: string; color?: string }) => (
        <svg
          key={tag.id}
          viewBox="0 0 24 24"
          width="11"
      ...
      {extra > 0 ? (
        <span className="event-tag-icons-extra" aria-hidden="true">
          +{extra}
        </span>
      ) : null}
    </span>
